...
docker-build:
	...
	docker build -f build/image/amd/Dockerfile -t registry.cn-shanghai.aliyuncs.com/guohz-sit/chaosblade-operator:${BLADE_VERSION} .

docker-build-arm64:
	GOOS="linux" GOARCH="arm64" go build $(GO_FLAGS) -o build/_output/bin/chaosblade-operator cmd/manager/main.go
	docker build -f build/image/arm/Dockerfile -t registry.cn-shanghai.aliyuncs.com/guohz-sit/chaosblade-operator-arm64:${BLADE_VERSION} .

push_image:
	docker push registry.cn-shanghai.aliyuncs.com/guohz-sit/chaosblade-operator:${BLADE_VERSION}
	docker push registry.cn-shanghai.aliyuncs.com/guohz-sit/chaosblade-operator-arm64:${BLADE_VERSION}

#operator-sdk 0.19.0 build
build_all_operator: pre_build build build_image
build_image:
	operator-sdk build --go-build-args="$(GO_FLAGS)" registry.cn-shanghai.aliyuncs.com/guohz-sit/chaosblade-operator:${BLADE_VERSION}

build_image_arm64:
	GOOS="linux" GOARCH="arm64" operator-sdk build --go-build-args="$(GO_FLAGS)" registry.cn-shanghai.aliyuncs.com/guohz-sit/chaosblade-operator-arm64:${BLADE_VERSION}
...
build_arm64:
	...
	docker run --rm \
		...
		-v $(shell pwd):/go/src/github.com/chaosblade-io/chaosblade-operator \
		-w /go/src/github.com/chaosblade-io/chaosblade-operator \
		registry.cn-shanghai.aliyuncs.com/guohz-sit/chaosblade-build-arm:latest
